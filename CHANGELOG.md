...
- Chore **(Major update)**: minimum required `stylelint-scss` version
  is now `^2.0.0`.

## 36.0.0 - 2017-07-18

- Chore **(Major update)**: minimum required `stylelint-order` version
  is now `^0.6.0`.
...
- Added **(Major update)**: `break-inside` property
  in `order/properties-order` rule.

- Added **(Major update)**: `css` preset.

- Chore **(Major update)**: minimum required `stylelint` version
  is now `^7.12.0`.

- Removed **(Major update)**: `core` preset (using for build-in purpose).

- Removed **(Major update)**: `selector-no-universal` rule 
  in favor `selector-max-universal`.
...
- Changed: properties order
  in `order/declaration-block-properties-specified-order`.
...
- Changed: set option `unspecified` to `ignore`
  in `order/declaration-block-properties-specified-order`.
...
- Changed: deleted characters length limit
  from `scss/percent-placeholder-pattern`.

## 21.0.0 - 2017-02-11

- Changed: moved `stylelint-order` rules to `core` preset.

- Changed: set `resolveNestedSelectors` to `true`
  in `selector-class-pattern` rule.
...
- Added **(Major update)**: `after-same-name` in `except` option 
  into `at-rule-empty-line-before` rule.
...
- Added **(Major update)**: `transition-property` with `all` value 
  in `declaration-property-value-blacklist` rule. 
...
- Changed: `declaration-block-semicolon-newline-after` rule 
  now have `always` option.

- Changed: `declaration-block-semicolon-space-after` rule now 
  have `null` option.

- Changed: `declaration-block-single-line-max-declarations` rule 
  now have `null` option.
...
- Chore **(Major upgrade)**: minimum required `stylelint` version
  is now `^7.4.2`.
...
- Chore **(Major upgrade)**: minimum required `stylelint` version
  is now `^7.3.1`. Version `7.4.0`, `7.4.1` is buggy.
...
- Chore **(Major update)**: minimum required `stylelint` version
  is now `^7.2.0`. Version `7.3.0` is buggy.
...
- Chore **(Major update)**: merge package `stylelint-config-itgalaxy`
  with `stylelint-config-itgalaxy-scss`.

- Chore: use `Object.assign` instead `merge` package.

- Chore: `devDependencies` and `peerDependencies` now have equals version
  of packages.

- Chore: update the minimal version of `npm-run-all` version is now `2.3.0`.

- Fixed: add `content` to `ignoreAtRules` in `at-rule-no-unknown` rule
  in `scss` preset.

## 8.0.0

- Changed(revert): use option `blockless-group` 
  instead `blockless-after-same-name-blockless` from `except` in
...
- Changed: use option `blockless-after-same-name-blockless` 
  instead `blockless-group` from `except` in `at-rule-empty-line-before` rule.
...
- Removed: option `all-nested` from `ignore` 
...
- Changed: rename `no-missing-eof-newline` 
  to `no-missing-end-of-source-newline`.

- Changed: rename `media-query-parentheses-space-inside`
